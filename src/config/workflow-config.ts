...
export function getFilteredWorkflowNodes(options: WorkflowConfigOptions = {}): WorkflowNodeConfig[] {
    ...
    if (options.includeCategories && options.includeCategories.length > 0) {
        filteredNodes = filteredNodes.filter(node => options.includeCategories!.includes(node.category));
    }
    ...
    if (options.excludeTypes && options.excludeTypes.length > 0) {
        filteredNodes = filteredNodes.filter(node => !options.excludeTypes!.includes(node.type));
    }

    // Include only specific types
    if (options.includeOnlyTypes && options.includeOnlyTypes.length > 0) {
        filteredNodes = filteredNodes.filter(node => options.includeOnlyTypes!.includes(node.type));
    }
...
export const WORKFLOW_PRESETS = {
    ...
    full_sequence: {
        ...
        allowedTypes: ['profile_visit', 'like_post', 'send_connection_request', 'send_followup', 'send_inmail'] as const,
        recommendedFlow: [
            { type: 'profile_visit', delay: 0 },
            { type: 'like_post', delay: 2 },
            { type: 'send_connection_request', delay: 24 },
...
export function validateWorkflowConfiguration(nodeTypes: string[]): {
    ...
    const hasConnectionRequest = nodeTypes.some(type => ['send_connection_request', 'send_invite'].includes(type));
...
export function getRecommendedNextSteps(currentTypes: string[]): WorkflowNodeConfig[] {
    ...
    if (currentTypesSet.has('profile_visit') && !currentTypesSet.has('like_post') && !currentTypesSet.has('comment_post')) {
    ...
    if ((currentTypesSet.has('like_post') || currentTypesSet.has('comment_post')) && !currentTypesSet.has('send_connection_request')) {
    ...
    if (currentTypesSet.has('send_connection_request') && !currentTypesSet.has('send_followup')) {
