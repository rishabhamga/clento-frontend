...
export const MakeAxiosRequest = async <RetType = any>(method: 'GET' | 'POST' | 'PUT' | 'PATCH' | 'DELETE', url: string, postBodyOrQueryParams: JsonType = {}, headers: JsonType = {}, timeout = 60_000, maxRedirects: number | undefined = undefined, httpAgent: any = null, httpsAgent: any = null, configParams: any = {}): Promise<AxiosResponse<RetType>> => {
    try {
...
export const makeAuthenticatedRequest = async <T = any>(method: 'GET' | 'POST' | 'PUT' | 'PATCH' | 'DELETE', endpoint: string, data: JsonType = {}, token: string, additionalHeaders: JsonType = {}, timeout = 60_000): Promise<T> => {
    ...
    const response = await MakeAxiosRequest<T>(method, url, data, headers, timeout);

    return handleApiResponse(response);
};

// Convenience function for public requests (no auth)
export const makePublicRequest = async <T = any>(method: 'GET' | 'POST' | 'PUT' | 'PATCH' | 'DELETE', endpoint: string, data: JsonType = {}, additionalHeaders: JsonType = {}, timeout = 60_000): Promise<T> => {
    const url = buildApiUrl(endpoint);
    const headers = {
        'Content-Type': 'application/json',
        ...(additionalHeaders as Record<string, string>),
...
export const makeFormDataRequest = async <T = any>(method: 'POST' | 'PUT' | 'PATCH', endpoint: string, formData: FormData, token: string, timeout = 60_000): Promise<T> => {
    const url = buildApiUrl(endpoint);
    const headers = {
        Authorization: `Bearer ${token}`,
    ...
    const response = await MakeAxiosRequest<T>(method, url, formData, headers, timeout);
