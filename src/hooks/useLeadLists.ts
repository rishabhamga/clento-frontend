...
import { CreateLeadListRequest, CsvPreviewResponse, LeadList, LeadListQuery, LeadListView, PublishLeadListRequest, PublishLeadListResponse, UpdateLeadListRequest } from '@/types/lead-list';
...
export function useLeadList(id: string) {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            return makeAuthenticatedRequest<LeadListView>('GET', `/lead-lists/detail?id=${id}`, {}, token);
        },
        enabled: !!id,
    });
}
...
export function useUpdateLeadList() {
    ...
    return useMutation({
        mutationFn: async ({ id, data }: { id: string; data: UpdateLeadListRequest }) => {
            ...
            const response = await makeAuthenticatedRequest<{ data: LeadList }>('POST', `/lead-lists/edit`, reqbody, token);
            return response.data;
        },
